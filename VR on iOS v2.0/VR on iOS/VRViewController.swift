...
class VRViewController: UIViewController, ARSessionDelegate, SCNSceneRendererDelegate {
    ...
    func session(_ session: ARSession, didAdd anchors: [ARAnchor]) {
        ...
        for anchor in anchors {
            ...
            if let imageAnchor = anchor as? ARImageAnchor {
                ...
                let plane = SCNSphere(radius: (referenceImage.physicalSize.width / 2) * CGFloat(multiplier)) // SCNPlane(width: referenceImage.physicalSize.width * CGFloat(multiplier), height: referenceImage.physicalSize.height * CGFloat(multiplier))
    ...
    func session(_ session: ARSession, didUpdate frame: ARFrame) {
        ...
        let imagePos = SCNVector3(
            (/*anchor.transform.columns.3.x + */node.position.x) * multiplier,
            (/*anchor.transform.columns.3.y + */node.position.y) * multiplier,
            (/*anchor.transform.columns.3.z + */node.position.z) * multiplier
        )
